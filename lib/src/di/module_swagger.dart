...
Future<void> registerApis(GetIt getIt) async {
  ...
  getIt.registerLazySingleton<DetailedEmployeeReportsApi>(
      () => _detailedEmployeeReportsApi());
  getIt.registerLazySingleton<SummaryEmployeeReportsApi>(
      () => _summaryEmployeeReportsApi());
...
Future<ApiClient> _apiClient() async {
  return ApiClient(
      ...
      client:
          RestClient(inject<AccessTokenInterceptor>(), inject<TokenManager>()));
}
...
DetailedEmployeeReportsApi _detailedEmployeeReportsApi() =>
    DetailedEmployeeReportsApi(inject<ApiClient>());

SummaryEmployeeReportsApi _summaryEmployeeReportsApi() =>
    SummaryEmployeeReportsApi(inject<ApiClient>());

UserClockInOutsApi _userClockInOutsApi() =>
    UserClockInOutsApi(inject<ApiClient>());
